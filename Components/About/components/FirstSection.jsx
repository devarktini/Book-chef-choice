...
<p className="text-base md:text-lg para text-primary">
    ...
    <div className={`${styles.sectionContainer} ${className} py-10`}>
      ...
      <div className="px-4 sm:px-8 md:px-12 pt-10 md:pt-24 grid gap-6 md:grid-cols-3">
        ...
        <div className="item1 flex flex-col items-center space-y-4 md:block">
          <div>
<p className="text-sm md:text-base para text-primary">
...
<p className={`${styles.cardHeading} text-primary`}>{item.type}</p>


                {/* Data lines in white + Roboto */}
                {Array.isArray(item.data) ? (
                  item.data.map((val, idx) => (
<p className={`${styles.cardData} text-primary`} key={idx}>

                      {val}
                    </p>
                  ))
                ) : (
<p className={`${styles.cardData} text-primary`}>{item.data}</p>
